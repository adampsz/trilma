...
export default class Board extends Base {
  ...
  constructor(root: SVGSVGElement, data: Data) {
    super();
    ...
    this.root.setAttribute('viewBox', `${-scale} ${-scale} ${2 * scale} ${2 * scale}`);
    this.makeBoard();
    ...
    this.root.addEventListener('click', (e) => {
      ...
      const [x, y] = target.getAttribute('data-xy')!.split(',').map(Number);

      if (target.getAttribute('href')!.includes('piece') && this.onPieceClick) this.onPieceClick([x, y]);
      if (target.getAttribute('href')!.includes('dot') && this.onDotClick) this.onDotClick([x, y]);
    });
  }

  intend(action: BoardAction): boolean {
    if (action.type === 'jump' || action.type === 'step') {
      const piece = this.root.querySelector(`.pieces [data-xy="${action.from[0]},${action.from[1]}"]`);
  ...
  hint(...xys: Vec2[]) {
    for (const hinted of this.root.querySelectorAll('.hinted')) hinted.classList.remove('hinted');

    for (const xy of xys) {
      const dot = this.root.querySelector(`.dots [data-xy="${xy[0]},${xy[1]}"]`);
  ...
  select(xy: Vec2 | null) {
    ...
    const piece = this.root.querySelector(`.pieces [data-xy="${xy[0]},${xy[1]}"]`);
  ...
  sync(data: Data) {
    ...
    for (const piece of this.root.querySelectorAll('.pieces [data-xy]')) {
      const [x, y] = piece.getAttribute('data-xy')!.split(',').map(Number);
    ...
    for (let i = 0; i < this.data.length; i++) {
      ...
      if (this.homes[i] === CELL_NONE)
        this.root.querySelector(`.dots [data-xy="${x},${y}"]`)?.removeAttribute('data-color');
      else this.root.querySelector(`.dots [data-xy="${x},${y}"]`)?.setAttribute('data-color', `${this.homes[i]}`);

      if (!present[i] && Board.isPiece(this.data[i]))
        this.root.querySelector('.pieces')!.appendChild(this.makePiece(x, y, this.data[i]));
    }
  }
  ...
  private makeBoard() {
    ...
    const path = this.root.querySelector('.outline')! as SVGPathElement;
    const dots = this.root.querySelector('.dots')! as SVGGElement;
    const pieces = this.root.querySelector('.pieces')! as SVGGElement;
