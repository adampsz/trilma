...
export default class Players implements Data {
  ...
  sync(data: Data) {
    ...
    data.all.forEach((data, i) => {
      const li =
        this.root.querySelector(`[data-player="${data.id}"]`) ||
        this.makePlayer();
      if (this.root.children[i] != li)
        this.root.insertBefore(li, this.root.children[i]);
      ...
      data.color
        ? color.setAttribute("data-color", `${data.color}`)
        : color.removeAttribute("data-color");
