...
export default class Game extends Base {
  ...
  constructor(socket: Socket, id: string, data: Data) {
    ...
    this.players = new Players(this.root.querySelector('.players')!, data.players);
    this.board = new Board(this.root.querySelector('.board')! as any, data.board);
    ...
    this.board.onDotClick = (xy) => {
      this.intend({ type: 'step', player: this.id, to: xy }) || this.intend({ type: 'jump', player: this.id, to: xy });
    };

    this.root.querySelector('button[name="ready"]')!.addEventListener('click', () => {
      this.intend({ type: 'ready', player: this.id });
    });

    this.root.querySelector('button[name="next"]')!.addEventListener('click', () => {
      this.intend({ type: 'next', player: this.id });
    });

    if (process.env.NODE_ENV === 'development') this.socket.onAny(console.debug);

    document.onkeydown = (e) => {
      if (e.key === ' ' || e.key === 'Enter') this.intend({ type: 'next', player: this.id });
    };
  ...
  intend(a: Action): boolean {
    ...
    if (a.type === 'jump' || a.type === 'step') {
      ...
      return !!color && this.board.intend({ ...a, color, from: this.state.piece });
    }
  ...
  update() {
    ...
    if (this.state.type !== 'waiting' || this.state.ready.includes(this.id)) ready.setAttribute('hidden', 'true');
    else ready.removeAttribute('hidden');

    const next = this.root.querySelector('[name="next"]')!;
    if ('player' in this.state && this.state.player === this.id) next.removeAttribute('hidden');
    ...
    if (!('player' in this.state) || this.state.player !== this.id || this.state.type === 'stepped') {
    ...
    if (this.state.type === 'selected') {
      ...
      const moves = [...this.board.steps(color, this.state.piece), ...this.board.jumps(color, this.state.piece)];
